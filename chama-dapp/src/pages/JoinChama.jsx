...
import { UPGRADED_CHAMA_FACTORY_ABI, UPGRADED_CHAMA_FACTORY_ADDRESS } from "../contracts/UpgradedChamaFactoryConfig";
import KrnlService from "../services/KrnlService";
...
const JoinChama = () => {
    ...
    const fetchChamas = async () => {
      ...
      try {
        ...
        const factoryContract = new Contract(
          UPGRADED_CHAMA_FACTORY_ADDRESS,
          UPGRADED_CHAMA_FACTORY_ABI,
          ethersProvider
        );
  ...
  const handleJoin = async () => {
    if (!selectedChama || !walletProvider || !address) return;

    try {
      setJoinLoading(true);

      // Step 1: Get KRNL validation payload
      console.log("[KRNL] Requesting validation for joining Chama");
      const krnlPayload = await KrnlService.getKrnlPayload(
        "joinChama",
        { chamaId: selectedChama.id },
        address
      );
      console.log("[KRNL] Received payload:", krnlPayload);

      // Step 2: Submit transaction with KRNL payload
      const ethersProvider = new BrowserProvider(walletProvider);
      ...
      const tx = await factoryContract.joinChama(selectedChama.id, krnlPayload, { value });
      ...
      alert(`Failed to join Chama: ${error.message || error}`);
    } finally {
