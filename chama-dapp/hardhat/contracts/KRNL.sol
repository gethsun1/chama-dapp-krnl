...
contract KRNL is Ownable {
    ...
    bool public devMode = true; // Set to true for development, false for production
    ...
    function _isAuthorized(KrnlPayload memory payload, bytes memory functionParams) private returns (bool) {
        // In development mode, bypass all validation
        if (devMode) {
            return true;
        }
        ...
        bytes32 dataDigest = keccak256(
            abi.encodePacked(functionParamsDigest, kernelParamObjectDigest, msg.sender, nonce, finalOpinion)
        );
